...
export default function TreasuryDashboard() {
  ...
  const createMut = useMutation({ mutationFn: createPaymentOrder, onSuccess: () => { toast.success("Ordre de paiement créé"); setSelectedExpense(null); qc.invalidateQueries({ queryKey: ["expenses"] }); qc.invalidateQueries({ queryKey: ["payment-orders"] }); qc.invalidateQueries({ queryKey: ["treasury-dashboard"] }); }, onError: (e) => toast.error(e?.response?.data?.message || "Erreur ordre de paiement") });
  ...
  const markPrintedMut = useMutation({ mutationFn: markPaymentOrderPrinted, onSuccess: (data) => { setPrintOrder(data.paymentOrder); qc.invalidateQueries({ queryKey: ["payment-orders"] }); qc.invalidateQueries({ queryKey: ["treasury-dashboard"] }); window.print(); }, onError: (e) => toast.error(e?.response?.data?.message || "Erreur impression") });
  ...
  return <div className="min-h-screen bg-gradient-to-br from-white via-emerald-50 to-white"><header className="border-b border-emerald-100 bg-white/80 backdrop-blur"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4"><Brand /><div className="flex items-center gap-3"><LanguageSwitcher /><span className="hidden text-xs font-bold text-slate-600 md:block">{user?.fullName || user?.email} • {t("treasury.roleLabel")}</span><AccountSecurityButton /><button onClick={logout} className="rounded-xl border border-emerald-200 px-3 py-2 text-xs font-bold">{t("common.logout")}</button></div></div></header><main className="mx-auto max-w-7xl px-4 py-8"><div className="mb-6 flex flex-wrap gap-2">{[["dashboard", "Dashboard"], ["expenses", "Liste des dépenses"], ["orders", "Ordres de paiement"]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`rounded-xl px-4 py-2 text-sm font-black ${tab === key ? "bg-emerald-600 text-white" : "bg-white text-slate-700"}`}>{label}</button>)}</div>{tab === "dashboard" ? <div className="space-y-6"><Card className="p-6"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-black text-slate-900">Dashboard trésorerie</h1><p className="mt-1 text-sm text-slate-500">Statistiques dépenses globales et ordres de paiement.</p></div>{qStats.isLoading ? <span className="text-sm text-slate-500">{t("common.loading")}</span> : null}{qStats.isError ? <span className="text-sm font-bold text-red-600">{t("common.loadingError")}</span> : null}</div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><StatCard label="Total dépenses" value={stats?.totalExpensesCount} /><StatCard label="Dépenses approuvées" value={stats?.approvedExpenses} /><StatCard label="Dépenses rejetées" value={stats?.rejectedExpenses} /><StatCard label="Dépenses en attente" value={stats?.pendingExpenses} /><StatCard label="Dépenses effectuées" value={stats?.completedExpenses} /><StatCard label="Dépenses du mois" value={stats?.monthExpensesCount} /><StatCard label="Montant total" value={fmtMoney(stats?.totalExpenses)} /><StatCard label="Montant du mois" value={fmtMoney(stats?.monthExpensesAmount)} /><StatCard label="Ordres de paiement" value={stats?.totalPaymentOrders} /><StatCard label="Ordres du mois" value={stats?.monthPaymentOrders} /><StatCard label="Montant ordres de paiement" value={fmtMoney(stats?.totalPaymentOrdersAmount)} /><StatCard label="Montant ordres du mois" value={fmtMoney(stats?.monthPaymentOrdersAmount)} /><StatCard label="Ordres créés" value={stats?.createdPaymentOrders} /><StatCard label="Ordres imprimés" value={stats?.printedPaymentOrders} /></div><div className="mt-6 grid gap-4 lg:grid-cols-2"><SimpleBarChart title="Ordres de paiement par statut" data={paymentOrderStatusChartData} emptyMessage="Aucun ordre de paiement par statut" /><SimplePieChart title="Dépenses approuvées/effectuées/rejetées" data={expenseStatusChartData} emptyMessage="Aucune dépense par statut" /></div></Card><div className="grid gap-6 lg:grid-cols-2"><Card className="p-6"><h2 className="mb-4 text-xl font-black text-slate-900">Dernières dépenses approuvées</h2><div className="space-y-3">{(stats?.latestApprovedExpenses || []).map((e) => <MiniItem key={e.id} title={e.label} badge={<Badge tone={statusTones[e.status]}>{t(`enumStatus.${e.status}`, e.status)}</Badge>}>{e.beneficiaryName} • {fmtMoney(e.amount)} • {fmtDate(e.createdAt)}</MiniItem>)}{(stats?.latestApprovedExpenses || []).length === 0 ? <div className="rounded-2xl border border-emerald-100 px-4 py-6 text-center text-sm text-slate-500">Aucune dépense approuvée</div> : null}</div></Card><Card className="p-6"><h2 className="mb-4 text-xl font-black text-slate-900">Derniers ordres de paiement</h2><div className="space-y-3">{(stats?.latestPaymentOrders || []).map((o) => <MiniItem key={o.id} title={o.referenceNumber} badge={<Badge tone={statusTones[o.status]}>{t(`enumStatus.${o.status}`, o.status)}</Badge>}>{o.expense?.beneficiaryName} • {fmtMoney(o.amount)} {t(`currencies.${o.currency}`, o.currency)}</MiniItem>)}{(stats?.latestPaymentOrders || []).length === 0 ? <div className="rounded-2xl border border-emerald-100 px-4 py-6 text-center text-sm text-slate-500">{t("paymentOrders.empty")}</div> : null}</div></Card></div></div> : null}{tab === "expenses" ? <Card className="p-6"><h1 className="mb-4 text-2xl font-black text-slate-900">{t("expenses.stats.approved")}</h1><div className="overflow-x-auto rounded-2xl border border-emerald-100"><table className="min-w-full divide-y divide-emerald-100 text-sm"><thead className="bg-emerald-50 text-left text-xs font-black uppercase text-slate-600"><tr>{["Date", "Libellé", "Bénéficiaire", "Pays", "Montant", "Statut", "Action"].map((h) => <th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody className="divide-y divide-emerald-50 bg-white">{expenses.map((e) => <tr key={e.id}><td className="px-4 py-3">{fmtDate(e.date)}</td><td className="px-4 py-3 font-semibold">{e.label}</td><td className="px-4 py-3">{e.beneficiaryName}</td><td className="px-4 py-3">{e.beneficiaryCountry}</td><td className="px-4 py-3 font-black">{fmtMoney(e.amount)}</td><td className="px-4 py-3"><Badge tone={statusTones[e.status]}>{t(`enumStatus.${e.status}`, e.status)}</Badge></td><td className="px-4 py-3"><button onClick={() => openOrder(e)} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white">Créer un ordre de paiement</button></td></tr>)}{!expenses.length ? <tr><td colSpan="7" className="px-4 py-6 text-center text-slate-500">Aucune dépense approuvée</td></tr> : null}</tbody></table></div></Card> : null}{tab === "orders" ? <Card className="p-6"><h1 className="mb-4 text-2xl font-black text-slate-900">Ordres de paiement</h1><div className="overflow-x-auto rounded-2xl border border-emerald-100"><table className="min-w-full divide-y divide-emerald-100 text-sm"><thead className="bg-emerald-50 text-left text-xs font-black uppercase text-slate-600"><tr>{["Référence", "Date", "Dépense", "Bénéficiaire", "Méthode", "Devise", "Pays", "Montant", "Status", "Action"].map((h) => <th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody className="divide-y divide-emerald-50 bg-white">{orders.map((o) => <tr key={o.id}><td className="px-4 py-3 font-black">{o.referenceNumber}</td><td className="px-4 py-3">{fmtDate(o.createdAt)}</td><td className="px-4 py-3">{o.expense?.label}</td><td className="px-4 py-3">{o.expense?.beneficiaryName}</td><td className="px-4 py-3">{t(`paymentMethods.${o.paymentMethod}`, o.paymentMethod)}</td><td className="px-4 py-3">{t(`currencies.${o.currency}`, o.currency)}</td><td className="px-4 py-3">{t(`countries.${o.paymentCountry}`, o.paymentCountry)}</td><td className="px-4 py-3 font-black">{fmtMoney(o.amount)}</td><td className="px-4 py-3"><Badge tone={statusTones[o.status]}>{t(`enumStatus.${o.status}`, o.status)}</Badge></td><td className="px-4 py-3"><button onClick={() => printMut.mutate(o.id)} className="rounded-xl border border-emerald-200 px-3 py-2 text-xs font-black">Imprimer</button></td></tr>)}{!orders.length ? <tr><td colSpan="10" className="px-4 py-6 text-center text-slate-500">{t("paymentOrders.empty")}</td></tr> : null}</tbody></table></div></Card> : null}</main><Drawer open={!!selectedExpense} onClose={() => setSelectedExpense(null)} title="Créer un ordre de paiement"><form onSubmit={submit} className="space-y-4"><Field label="Token reçu du Super Admin"><Input value={form.token} onChange={(e) => setForm({ ...form, token: e.target.value })} required /></Field><Field label="Méthode de paiement"><Select value={form.paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}><option value="VIREMENT_BANCAIRE">Virement bancaire</option><option value="CASH">Cash</option><option value="CHEQUE">Chèque</option></Select></Field><Field label="Devise"><Select value={form.currency} onChange={(e) => setForm({ ...form, currency: e.target.value })}><option value="FRANC">Franc</option><option value="DOLLAR">Dollar</option><option value="BIRR_ETHIOPIEN">Birr Ethiopien</option></Select></Field><Field label="Pays de paiement"><Select value={form.paymentCountry} onChange={(e) => setPaymentCountry(e.target.value)}><option value="DJIBOUTI">Djibouti</option><option value="ETHIOPIE">Ethiopie</option></Select></Field><Field label="Montant"><Input type="number" min="1" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} required /></Field>{form.paymentMethod !== "CASH" ? <><Field label="Nom de la banque"><Select value={form.bankName} onChange={(e) => setForm({ ...form, bankName: e.target.value })}>{bankOptionsByCountry[form.paymentCountry || "DJIBOUTI"].map((bank) => <option key={bank} value={bank}>{bank}</option>)}</Select></Field><Field label="Référence bancaire / numéro chèque"><Input value={form.bankReference} onChange={(e) => setForm({ ...form, bankReference: e.target.value })} required /></Field><Field label="Nom du titulaire / bénéficiaire bancaire"><Input value={form.bankAccountHolder} onChange={(e) => setForm({ ...form, bankAccountHolder: e.target.value })} /></Field></> : null}<button disabled={createMut.isPending} className="w-full rounded-xl bg-emerald-600 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{t("common.save")}</button></form></Drawer><Drawer open={!!printOrder} onClose={() => setPrintOrder(null)} title="Ordre de paiement imprimable" right={<button onClick={printCurrentOrder} disabled={markPrintedMut.isPending} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-50">Imprimer</button>}><div className="print:bg-white rounded-2xl border border-emerald-100 p-6 text-slate-900"><div className="text-center"><img src="/logoherciise.jpeg" alt="Hiil Foundation" className="mx-auto h-20 w-20 rounded-full object-cover" /><h2 className="mt-3 text-2xl font-black">Hiil Foundation</h2><p className="text-sm text-slate-500">Ordre de paiement</p></div><div className="mt-6 grid gap-3 text-sm"><p><b>Référence :</b> {printOrder?.referenceNumber}</p><p><b>Date :</b> {fmtDate(printOrder?.createdAt)}</p><p><b>Dépense :</b> {printOrder?.expense?.label}</p><p><b>Bénéficiaire :</b> {printOrder?.expense?.beneficiaryName}</p><p><b>Méthode :</b> {printOrder?.paymentMethod}</p><p><b>Devise :</b> {printOrder?.currency}</p><p><b>Pays :</b> {printOrder?.paymentCountry}</p><p><b>Montant :</b> {fmtMoney(printOrder?.amount)}</p></div><div className="mt-14 flex justify-between text-sm font-bold"><span>Signature trésorerie</span><span>Signature validation</span></div></div></Drawer></div>;
}
