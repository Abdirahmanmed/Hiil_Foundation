...
function MiniList({ title, items, render, t }) {
  return <Card className="p-6"><h2 className="mb-4 text-xl font-black text-slate-900">{title}</h2><div className="space-y-3">{items.map(render)}{!items.length ? <div className="rounded-2xl border border-emerald-100 px-4 py-6 text-center text-sm text-slate-500">{t("common.noData")}</div> : null}</div></Card>;
}
